...
const StatCard = ({ stat }) => {  
  ...
  return (
    <Card className="p-4 flex items-center gap-4 hover:shadow-md transition-shadow duration-200 border-0 shadow-sm">
      ...
      <div>
        <p className="text-2xl font-bold leading-none">{stat?.value ? stat?.value : 0}</p>
        <p className="text-xs text-muted-foreground mt-1">{stat?.name ? stat?.name : ''}</p>
      </div>
    </Card>
  );
}

const StatCards = ({ title, stats }) => {
  return (
    <div>
